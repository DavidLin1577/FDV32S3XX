...
void hexdump(int address, int len)
{
    ...
    if(len > 0 && len < 100)
    {
    	for(i = 0; i < len; i++)
    	{
    		shellDisplay(shell, "address  ");
    		shellDisplayValue(shell, address+i*4);
    		shellDisplay(shell, "value    ");
    		shellDisplayValue(shell, REG32(address+i*4));
    	}
    }
    else
    {
		shellDisplay(shell, "address  ");
		shellDisplayValue(shell, address);
		shellDisplay(shell, "value    ");
		shellDisplayValue(shell, REG32(address));
    }
}
SHELL_EXPORT_CMD(hexdump, hexdump, show the value of address);

int efcwrite(int address, int value)
{
	int i = 0;
	int ret = 0;
    ...
    if(address >= 0x2000 && address <= 0x67FF)
    {
        ret = EFC_SingleProgram(address, EFC_PRG_WORD, value);
    }

    if(address >= 0x7000 && address <= 0x70FF)
    {
        ret = EFC_EEPROMWrite(address, EFC_PRG_WORD, value);
    }

   return ret;

}
SHELL_EXPORT_CMD(efcwrite, efcwrite, write the value into address);
